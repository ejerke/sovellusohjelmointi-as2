...
int client_init(struct session *c,const char *bin) {
    ...
    if ( pid == 0 ) {
        close(STDIN_FILENO); dup(pout[0]); close(pout[0]); close(pout[1]);
        close(STDOUT_FILENO); dup(pin[1]); close(pin[1]); close(pin[0]);
        execl(bin,bin,NULL);
...
int client_reap_pid(struct session *t,int max,int pid) {
    ...
    printf("Couldn't fild client with pid %d to reap\n",pid);
    return(-1);
}
...
void client_newcmd(struct session *c,int i) {
    ...
    case 5:
        ...
        break;
    case 6: // transfer
        len = snprintf(cmdbuf,CMDBUFSIZ,"t %d %d %d\n",(int)random() % 20 +1,(int)random() % 20 +1, (int)random() % 100 +1);
        c->state = cmdsent;
        break;
    case 7: // quit
    ...
    write(c->fdout,cmdbuf,len);
}
/**
 * Send quit command.
 * \param c Client structure.
 * \param i Client index (for printouts only). */
void client_cmdquit(struct session *c,int i) {
    const char *cmdquit = "q\n";
    c->state = exiting;
    printf("#%d: Sending command: %s",i,cmdquit);
    write(c->fdout,cmdquit,strlen(cmdquit));
}
...
//        printf("select() returned: %d\n",res);
        ...
        printf("cmds: %d, clients: %d\n",commands_to_run,numclients);
    }
